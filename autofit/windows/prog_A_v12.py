...
version 12 features (still in progress):
...
if __name__ == '__main__': #multiprocessing imports script as module
    ...
    fieldNames = ["a dipole (%s)"%(u_A),"b dipole (%s)"%(u_B),"c dipole (%s)"%(u_C),"A (%s MHz)"%(A),"B (%s MHz)"%(B),"C (%s MHz)"%(C),"-DJ (%s MHz)"%(DJ),"-DJK (%s MHz)"%(DJK),"-DK (%s MHz)"%(DK),"-dJ (%s MHz)"%(dJ),"-dK (%s MHz)"%(dK),\
                  "number of processors (%s)"%(processors),"upper intensity limit on exp spectrum (%s)"%(inten_high),"lower intensity limit on exp spectrum(%s)"%(inten_low),"temperature (%sK)"%(temp), "Jmax (%s)"%(Jmax)]
    ...
    for i in range(0,total_check_num-2):
        for j in range(i+1,total_check_num-1):
            for k in range(j+1,total_check_num):
                ...
                trans_2 = full_list[j]
                trans_3 = full_list[k]
                dependence = abs(dependence_test(float(A),float(B),float(C),float(DJ),float(DJK),float(DK),float(dJ),float(dK),trans_1,trans_2,trans_3,temp,freq_high,freq_low,u_A,u_B,u_C))
    ...
    for entry in scaled_triples_scores:
        ...
        triples_choice_list.append((entry[10],entry[9],entry[11],trans_1[1],trans_1[2],trans_1[3],trans_2[1],trans_2[2],trans_2[3],trans_3[1],trans_3[2],trans_3[3],entry[0],entry[1],entry[2]))

...
def on_key( event ):
    if event.key=="q" or event.key=="Q":
        ...
        #print x
        difference_list = []
        new_text = "Most Recent Selection: \n"
...
def cat_reader(freq_high=25000,freq_low=0): #reads output from SPCAT
    fh = open("default.cat")
    linelist = []
    for line in fh:
        if line[8:9]==".": 
...
def update(val):
    ...
    int_writer(ua_g,ub_g,uc_g,temp=T_g)
    var_writer_uncert(A_value,B_value,C_value,DJ_g,DJK_g,DK_g)
    run_SPCAT()
    data = cat_reader()
    s = []
    t = []
...
def run_main(A,B,C,dA,dB,dC,DJ,DJK,DK,ua,ub,uc,T,last_times_picked_list,number_of_skips,spectrum_filename=None):

...
############################<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
	...
	#print fitting_GUI_B_v9.picked_list
	full_list = [(entry[3],entry[0],entry[1],entry[2],entry[4]) for entry in fitting_GUI_B_v9.picked_list] 
	if len(full_list)<3:
	    msg = "You need to choose at least three transitions before doing an automatic setup."
            ...
            counter = 0
            self.fit_list = []
            for entry in fitting_GUI_B_v9.picked_list:
                if entry[1]==best_option[1][1] and entry[2]==best_option[1][2]:
                    #print entry[1],best_option[1][1],entry[2],best_option[1][2]
                    fitting_GUI_B_v9.picked_list[counter] = (entry[0],entry[1],entry[2],entry[3],entry[4],'FIT')
                    ...
                    self.fit_list.append((entry[0],entry[1],entry[2],entry[3],entry[4]))
                else: fitting_GUI_B_v9.picked_list[counter] = (entry[0],entry[1],entry[2],entry[3],entry[4],'CHECK')
                counter+=1                  
            ...
            #fitting_GUI_B_v9.triples_plt.set_data([],[])
            try:
                ...
                if num_of_triples!=0:
                    ...
                    self.num_of_triples_label.setStyleSheet('color: green')
                else:
    ...
    def update_fit_check_status(self):
        ...
        for line in fitting_GUI_B_v9.picked_list:
            if line[5]=='CHECK':
                check_counter+=1
            if line[5]=='FIT':
                fit_counter+=1            
        ...
        if check_counter!=0:
            ...
            self.check_status_label2.setStyleSheet('color: green')
            check_flag = 1
        if check_counter==0:
        ...
        if fit_counter==3:
            ...
            self.fitting_status_label2.setStyleSheet('color: green') 
            fit_flag = 1          
        if fit_counter!=3:
    ...
    def update_exp_spectrum(self):
        ...
        try:
            ...
            fitting_GUI_B_v9.exp_plt.set_data(spectrum_list_F,spectrum_list_I)
            
            new_list.sort()
            fitting_GUI_B_v9.ax2.set_ylim([float(new_list[0]),float(new_list[-1])])
            fitting_GUI_B_v9.plt.show()
            fh.close()
    ...
    def plot_triples(self):
        ...
        if self.plot_triples_cb.isChecked()==True:
            ...
            fitting_GUI_B_v9.triples_plt.set_data(f_list,i_list)
            fitting_GUI_B_v9.plt.show()
            ...
            for line in self.trans_3_peaks:
                F_3.append(line[0])
                I_3.append(line[1])
            fitting_GUI_B_v9.trans_1_plt.set_data(F_1,I_1)
            fitting_GUI_B_v9.trans_2_plt.set_data(F_2,I_2)
            fitting_GUI_B_v9.trans_3_plt.set_data(F_3,I_3)
            fitting_GUI_B_v9.plt.show()
        ...
        if self.plot_triples_cb.isChecked()==False:
            ...
            fitting_GUI_B_v9.trans_1_plt.set_data([],[])
            fitting_GUI_B_v9.trans_2_plt.set_data([],[])
            fitting_GUI_B_v9.trans_3_plt.set_data([],[])       
            fitting_GUI_B_v9.triples_plt.set_data([],[])
            fitting_GUI_B_v9.plt.show()
    def replot_button(self):
        ...
        if self.counter != 0:
            picked_list = fitting_GUI_B_v9.picked_list
        ...
        fitting_GUI_B_v9.run_main(A,B,C,dA,dB,dC,DJ,DJK,DK,dJ,dK,ua,ub,uc,f_lower,f_upper,T,J_max,picked_list)
...
str(freq_low),str(inten_high),str(inten_low),str(temperature),str(Jmax),str(self.num_of_triples),check_peaks_str,str(trans_1),str(trans_2),str(trans_3))
        ...
        for line in fh_input:
                    ...
                    if line.split() != []:
                        ...
                        if line.split()[0] == "trans_1:" or line.split()[0] == "trans_2:" or line.split()[0] == "trans_3:":
                            ...
                            tuples = tuple(re_split)
                            print tuples
                            fitting_GUI_B_v9.picked_list.append((tuples[1],tuples[2],tuples[3],tuples[0],tuples[4],'FIT'))
                        ...
                        if fitting_peaks_flag == 1:
                            ...
                            tuples = tuple(re_split)
                            #print tuples
                            fitting_GUI_B_v9.picked_list.append((tuples[1],tuples[2],tuples[3],tuples[0],tuples[4],'CHECK'))
                        if line.split()[0] == "Check":
                            fitting_peaks_flag = 1
        #self.peaks_button_push()
        fh_input.close()
        self.peaks_button_push()
        for lin in fitting_GUI_B_v9.picked_list:
            print lin
        fitting_GUI_B_v9.update_plot()
        self.update_fit_check_status()
    ...
    def peak_pick_button(self):
        ...
        self.peakfinder_status_label2.setStyleSheet('color: green')       
        self.update_triples_meter()
    def on_about(self):
    ...
    def delete_button_push(self):
            counter = 0
            new_picked_list = []
            for x in fitting_GUI_B_v9.picked_list:
                if self.model.item(counter).checkState()==0:
                    ...
                    pass
                counter+=1                                                      
            fitting_GUI_B_v9.picked_list = new_picked_list    
    ...
    def update_triples_labels(self):
            ...
            for new_entry in fitting_GUI_B_v9.picked_list:
    ...
    def plot_current_peaklist(self):
        if self.peak_center_cb.isChecked()==True:
            ...
            fitting_GUI_B_v9.peak_list_plt.set_data(self.peaklist[:,0],self.peaklist[:,1]) 
            
            fitting_GUI_B_v9.ax2.set_ylim([0,self.peaklist.max(0)[1]])    
            fitting_GUI_B_v9.plt.show()          
        if self.peak_center_cb.isChecked()==False:
            
            fitting_GUI_B_v9.peak_list_plt.set_data([],[]) 
            fitting_GUI_B_v9.plt.show()  

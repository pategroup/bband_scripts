...
class AppForm(QMainWindow):
	...
	full_list = [(entry[3],entry[0],entry[1],entry[2],entry[4]) for entry in fitting_GUI_B_v6.picked_list] 
	if len(full_list)<3:
	    msg = "You need to choose at least three transitions before doing an automatic setup."
            ...
            for entry in fitting_GUI_B_v6.picked_list:
                if entry[1]==best_option[1][1] and entry[2]==best_option[1][2]:
                    ...
                    fitting_GUI_B_v6.picked_list[counter] = (entry[0],entry[1],entry[2],entry[3],entry[4],'FIT')
                    ...
                    self.fit_list.append((entry[0],entry[1],entry[2],entry[3],entry[4]))
                else: fitting_GUI_B_v6.picked_list[counter] = (entry[0],entry[1],entry[2],entry[3],entry[4],'CHECK')
                counter+=1                  
        ...
        for line in fh_input:
                    ...
                    if line.split() != []:
                        ...
                        if line.split()[0] == "trans_1:" or line.split()[0] == "trans_2:" or line.split()[0] == "trans_3:":
                            ...
                            tuples = tuple(re_split)
                            print tuples
                            fitting_GUI_B_v6.picked_list.append((tuples[1],tuples[2],tuples[3],tuples[0],tuples[4],'FIT'))
                            self.peaks_button_push()
                        if fitting_peaks_flag == 1:
                            ...
                            tuples = tuple(re_split)
                            #print tuples
                            fitting_GUI_B_v6.picked_list.append((tuples[1],tuples[2],tuples[3],tuples[0],tuples[4],'CHECK'))
	...
	for line in fitting_GUI_B_v6.picked_list:
	    print line

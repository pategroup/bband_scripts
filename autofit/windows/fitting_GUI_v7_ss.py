...
class AppForm(QMainWindow):
	...
	self.uncertainty_box.setText('100.0')
	...
	full_list = [(entry[3],entry[0],entry[1],entry[2],entry[4]) for entry in fitting_GUI_B_v7.picked_list] 
	if len(full_list)<3:
	    msg = "You need to choose at least three transitions before doing an automatic setup."
            ...
            for line in results:
                ...
                if 0==self.check_bounds(float(line[3]),float(line[6]),float(line[9]),uncertainty,uncertainty,uncertainty,freq_low,freq_high)[0]:
            ...
            counter = 0
            self.fit_list = []
            for entry in fitting_GUI_B_v7.picked_list:
                if entry[1]==best_option[1][1] and entry[2]==best_option[1][2]:
                    #print entry[1],best_option[1][1],entry[2],best_option[1][2]
                    fitting_GUI_B_v7.picked_list[counter] = (entry[0],entry[1],entry[2],entry[3],entry[4],'FIT')
                    ...
                    self.fit_list.append((entry[0],entry[1],entry[2],entry[3],entry[4]))
                else: fitting_GUI_B_v7.picked_list[counter] = (entry[0],entry[1],entry[2],entry[3],entry[4],'CHECK')
                counter+=1                  
            self.peaks_button_push()
            window_decision='User-defined, same for each'
            peak_1_uncertainty = uncertainty
            peak_2_uncertainty = uncertainty
            peak_3_uncertainty = uncertainty
    ...
    def replot_button(self):
        ...
        if self.counter != 0:
            picked_list = fitting_GUI_B_v7.picked_list
        ...
        try:
            try:
                ...
                fitting_GUI_B_v7.run_main(A,B,C,dA,dB,dC,DJ,DJK,DK,dJ,dK,ua,ub,uc,f_lower,f_upper,T,picked_list,int(unicode(self.skip_box.text())),self.filename,peaklist)
            except AttributeError:
                fitting_GUI_B_v7.run_main(A,B,C,dA,dB,dC,DJ,DJK,DK,dJ,dK,ua,ub,uc,f_lower,f_upper,T,picked_list,int(unicode(self.skip_box.text())),self.filename)
        except AttributeError:
            fitting_GUI_B_v7.run_main(A,B,C,dA,dB,dC,DJ,DJK,DK,dJ,dK,ua,ub,uc,f_lower,f_upper,T,picked_list,int(unicode(self.skip_box.text())))
        ...
        for line in fh_input:
                    ...
                    if line.split() != []:
                        ...
                        if line.split()[0] == "trans_1:" or line.split()[0] == "trans_2:" or line.split()[0] == "trans_3:":
                            ...
                            tuples = tuple(re_split)
                            print tuples
                            fitting_GUI_B_v7.picked_list.append((tuples[1],tuples[2],tuples[3],tuples[0],tuples[4],'FIT'))
                            self.peaks_button_push()
                        if fitting_peaks_flag == 1:
                            ...
                            tuples = tuple(re_split)
                            #print tuples
                            fitting_GUI_B_v7.picked_list.append((tuples[1],tuples[2],tuples[3],tuples[0],tuples[4],'CHECK'))
	...
	for line in fitting_GUI_B_v7.picked_list:
	    print line
    ...
    def delete_button_push(self):
            counter = 0
            new_picked_list = []
            for x in fitting_GUI_B_v7.picked_list:
                if self.model.item(counter).checkState()==0:
                    ...
                    pass
                counter+=1                                                      
            fitting_GUI_B_v7.picked_list = new_picked_list    
	...
	self.uncertainty_box = QLineEdit()
        self.plot_triples_cb = QCheckBox('Plot Triples', self)
	...
	hbox.addWidget(self.uncertainty_label,3,7)
	hbox.addWidget(self.uncertainty_box,3,8)
	...
	hbox.addWidget(self.dK_box,13,2)	
        ...
        hbox.addWidget(self.fix_cb,4,7)		
	...
	hbox.addWidget(self.plot_triples_cb,5,7)
        vbox = QVBoxLayout()
        vbox.addLayout(hbox)
